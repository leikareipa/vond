...
std::vector<triangle_s> kmesh_mesh_triangles(const char *const meshFilename)
{
    ...
    std::unordered_map<std::string/*material name*/, polygon_material_s> knownMaterials;
    ...
    while (!meshFile.file_is_at_end())
    {
        ...
        switch (line.command)
        {
            case '?':       // Comment line.
            {
                ...
                DEBUG(("Found a comment in the mesh file."));
                break;
            }
            case 'f':       // File format.
            {
            ...
            case 'o':       // Object.
            {
                DEBUG(("Found an object in the mesh file."));
                       ...
                       line.indentLevel == (parentIndentLevel + 1))
                {
                    ...
                    triangle_s tri;
                    polygon_material_s material = knownMaterials.at(line.params.at(0));
